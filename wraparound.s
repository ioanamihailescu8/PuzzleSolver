...
MAIN_WHILE_LOOP: 
      ...
      move $s7, $t5                   # $s7 is the strfind's function parameter idx_2
      j STRFIND       
                      # call strfind(idx_2, row)     
RETURN_FROM_STRFIND:
...
FOR_LOOP:      
       ...
       la $s2, grid($s7)               # string = grid[grid_idx]    
HORIZONTAL:              
       la $t6, ($t9)
       move $a3, $t6
...
CONTAIN_H:
       ...
       beq  $t3, $v1, CHECK_WRAPAROUND_H    # if (*string == '\n') reached end of line and we want to check if the word is wrap-around
 
CONTINUE_WRAPAROUND_H:     
...
CHECK_WRAPAROUND_H:
       addi $v1, $0, 10                # newline \n
       beq  $t1, $v1, PRINT_H          # if (*word=='\n') print it because it is not a wrap-around
       sub $a1, $a1, $s3               # string -= nr_cols
       lb $t3, 0($a1)   
       j CONTINUE_WRAPAROUND_H  
...
CONTAIN_V:
       ...
       lb $t1, 0($a2)                  # a2 is *word   -> get first character
       la $t2, grid
       sub $t2, $a1, $t2               # string - grid 
       addi $v0, $s3, 1                # nr_cols+1
       mul $v0, $v0, $s4               # (nr_cols+1)*nr_rows
       bge $t2, $v0, CHECK_WRAPAROUND_V  # reached final row and we want to check if the word is wrap-around

CONTINUE_WRAPAROUND_V:    
       bne $t3, $t1, END_CONTAIN_V     # if (*string != *word) go to label
       addi $a2, $a2, 1                # increase pointer word
       add $a1, $a1, $s3               
       addi $a1, $a1, 1                # string += num_cols + 1
       j CONTAIN_V

CHECK_WRAPAROUND_V:
       addi $v1, $0, 10                # newline \n
       beq  $t1, $v1, PRINT_V          # if (*word=='\n') print it because it is not a wrap-around
       addi $v1, $s3, 1                # num_cols +1
       mul $v1, $v1, $s4               #
       sub $a1, $a1, $v1               # string -= nr_rows*(nr_cols+1)
       lb $t3, 0($a1)                  # get first character from string
       j CONTINUE_WRAPAROUND_V   
...
CONTAIN_D:
       ...
       sub $t2, $a1, $t2               # string - grid 
       addi $v1, $s3, 1                # num_cols + 1
       div $v1, $t2, $v1               # Hi contains the remainder, Lo contains quotient
       mfhi $a0                        # remainder moved into $a0  = column
       mflo $v0                        # quotient moved into $v0   = row
       # the word we are checking should be the diagonal of a square with dimensions min(col, row)
       # so we need to find that minimum
       blt $v0, $a0, SET_MIN           # if row < column
       move $s0, $a0                   # set the min to be the column
       
BACK:              
       beq $v0, $s4, CHECK_WRAPAROUND_D
       beq $a0, $s3, CHECK_WRAPAROUND_D

       #la $t2, grid
       #sub $t2, $a1, $t2               # string - grid 
       #addi $v1, $s3, 1                # num_cols + 1
       #beq $v0, $v1, $s4               # nr_cols*nr_rows
       #  move to position P(row-min, col-min,)
       #bgt $t2, $v0, CHECK_WRAPAROUND_D    # if ( string - grid > (nr_cols+1)*nr_rows)
       #j CHECK_SECOND_CONDITION
       
CONTINUE_WRAPAROUND_D:    
       bne $t3, $t1, END_CONTAIN_D     # if (*string != *word) go to label
       addi $a2, $a2, 1                # increase pointer word
       add $a1, $a1, $s3
       addi $a1, $a1, 2                # string += num_cols + 2
       j CONTAIN_D

#CHECK_SECOND_CONDITION:
 #      addi $v1, $0, 10                # newline \n
  #     seq $v1, $t1, $v1               # 
   #    bne $v1, $0, CHECK_WRAPAROUND_D # if (*string == '\n')
    #   j CONTINUE_WRAPAROUND_D         # else 

CHECK_WRAPAROUND_D:
      addi $v1, $0, 10                 # newline \n
       beq  $t1, $v1, PRINT_D          # if (*word=='\n') print it because it is not a wrap-around
       addi $v1, $s3, 2                # num_cols + 2 
       mul $v1, $v1, $s0               #
       sub $a1, $a1, $v1               # string -= min*(nr_cols+2)      
       lb $t3, 0($a1)                  # get first character from string
       j CONTINUE_WRAPAROUND_D   

SET_MIN:
       move $s0, $v0                   #set the min to be the row
       j BACK
...
#     check if at least one word was found in the grid 
