...
int contain_v(char *string, char *word){
 while(1){  
    ...
    if ( string - grid >= (nr_cols+1)*nr_rows){
       if (*word == '\n') return 1;
       string -= nr_rows*(nr_cols+1);
    }
...
int contain_d(char *string, char *word){
   ...
   while(1){  
     int min = (((string - grid)/(nr_cols + 1)) < ((string - grid)%(nr_cols + 1))) ? ((string - grid)/(nr_cols + 1)) : ((string - grid)%(nr_cols + 1));
     if ( ((string - grid)/(nr_cols + 1))==nr_rows || ((string - grid)%(nr_cols + 1))==nr_cols){
       if (*word == '\n') return 1;
       string -= min*(nr_cols+2);
    }
